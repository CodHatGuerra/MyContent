...
async function onclick_DeleteMerchantChannel(salesChannel_ID) {
  ...
  if(result == true) {
    $.ajax({
        type: "DELETE",
        url: `${URL}/delivery-gateway-api/api/merchant/${$('#select_Merchant').val()}/salesChannel/${salesChannel_ID}`,
...
async function onclick_DeleteParameter(merchant_ID, sales_ID,parameter_ID) {
  ...
  if(result == true) {
    $.ajax({
        type: "DELETE",
        url: `${URL}/delivery-gateway-api/api/merchant/${merchant_ID}/salesChannel/${sales_ID}/parameter/${parameter_ID}`,
...
function onchange_MerchantChannel() {
  ...
  table_SalesChannel = $('#table_SalesOptions').DataTable({
      ajax: {
          type: "GET",
          dataType: "json",
          url: `${URL}/delivery-gateway-api/api/merchant/${$('#select_Merchant').val()}/salesChannel`,
          dataSrc: '',
...
async function onclick_New() {
  if( $('#select_Channel').has('option').length > 0 ) {
    await $.ajax({
      type: "GET",
      url: `${URL}/delivery-gateway-api/api/salesChannel`,
      dataType: "json",
  ...
  if( $('#select_Application').has('option').length > 0 ) {
    await $.ajax({
      type: "GET",
      url: `${URL}/delivery-gateway-api/api/application`,
      dataType: "json",
      success : function (data) {
        let response_Data = data;
...
function onclick_SaveNewSalesChannel() {
    $.ajax({
        type: "POST",
        dataType: "json",
        url: `${URL}/delivery-gateway-api/api/merchant/${$('#select_MerchantModal').val()}/salesChannel`,
        contentType: "application/json; charset=utf-8",
        data: JSON.stringify({
...
function onclick_SaveParameter() {
  $.ajax({
    type: "POST",
    dataType: "json",
    url: `${URL}/delivery-gateway-api/api/merchant/${$('#select_MerchantModal').val()}/salesChannel/${$('#select_Channel').val()}/parameter`,
    contentType: "application/json; charset=utf-8",
    data: JSON.stringify({
      Key: $('#key').val(),
      Value: $('#value').val()
  ...
  if(table_Parameters == null) {
    ...
    table_Parameters = $('#table_Parameters').DataTable({
        ajax: {
            type: "GET",
            dataType: "json",
            url: `${URL}/delivery-gateway-api/api/merchant/${merchant_ID}/salesChannel/${sales_ID}/parameter`,
            dataSrc: '',
            contentType: "application/json; charset=utf-8"
        },
        columns: colunas,
